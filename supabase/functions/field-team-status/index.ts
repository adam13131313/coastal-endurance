...
// Caller's own email + verified-authenticated role, from their Supabase JWT.
function emailFromAuth(header: string | null): string | null {
  ...
  try {
    ...
    return c?.role === "authenticated" && typeof c.email === "string" ? c.email.toLowerCase() : null;
  } catch {
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  // Any signed-in user can check their OWN membership. No admin rights needed.
  const email = emailFromAuth(req.headers.get("Authorization"));
  if (!email) return json({ member: false });

  try {
    const admin = createClient(SUPABASE_URL, SERVICE_ROLE_KEY, { auth: { persistSession: false } });
    const { data } = await admin.from("approved_field_team_members").select("email").eq("email", email).maybeSingle();
    return json({ member: !!data });
  } catch (e) {
    console.error("field-team-status error", e);
